...
// Build alliance analysis locally from scouting_submissions instead of using the prediction API.
if (!empty($selected_event) && !empty($selected_robot)) {

    // Pull per-robot summary stats for this event.
    $sql = "
        SELECT
            robot,
            COUNT(DISTINCT match_no) AS matches,
            COALESCE(SUM(points), 0) AS total_points,
            COALESCE(SUM(points) / NULLIF(COUNT(DISTINCT match_no), 0), 0) AS avg_points_per_match,
            COALESCE(SUM(CASE WHEN result = 'success' AND points > 0 THEN 1 ELSE 0 END), 0) AS scoring_successes,
            COALESCE(SUM(CASE WHEN time_sec <= 15 AND result = 'success' AND points > 0 THEN 1 ELSE 0 END) / NULLIF(COUNT(DISTINCT match_no), 0), 0) AS auton_score,
            COALESCE(SUM(CASE WHEN LOWER(action) LIKE '%def%' THEN 1 ELSE 0 END) / NULLIF(COUNT(DISTINCT match_no), 0), 0) AS defensive_impact_delta,
            MAX(CASE WHEN action IN ('algae_net','algae_processor') AND result = 'success' THEN 1 ELSE 0 END) AS algae_scored
        FROM scouting_submissions
        WHERE event_name = :event
        GROUP BY robot
        ORDER BY CAST(robot AS UNSIGNED)
    ";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(['event' => $selected_event]);
    $robotStats = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Build most common action map per robot.
    $actionSql = "
        ...
        ORDER BY CAST(robot AS UNSIGNED), action_count DESC, action ASC
    ";
    $stmt = $pdo->prepare($actionSql);
    $stmt->execute(['event' => $selected_event]);
    $actionRows = $stmt->fetchAll(PDO::FETCH_ASSOC);

    $mostCommonAction = [];
    foreach ($actionRows as $row) {
        $r = (string)$row['robot'];
        if (!isset($mostCommonAction[$r])) {
            $mostCommonAction[$r] = $row['action'];
        }
    }

    // Convert to scored candidates excluding the selected robot.
    $candidates = [];
    foreach ($robotStats as $row) {
        $robotNum = (string)$row['robot'];
        if ($robotNum === (string)$selected_robot) {
            continue;
        }

        $matches = (int)$row['matches'];
        $avgPoints = (float)$row['avg_points_per_match'];
        $scoringSuccesses = (int)$row['scoring_successes'];

        // Simple local cycle estimate: 150 / average successful scoring actions per match
        $successesPerMatch = $matches > 0 ? ($scoringSuccesses / $matches) : 0;
        $baselineCycle = $successesPerMatch > 0 ? (150 / $successesPerMatch) : 999;

        $autonScore = (float)$row['auton_score'];
        $defImpact = (float)$row['defensive_impact_delta'];
        $favoriteAction = $mostCommonAction[$robotNum] ?? 'N/A';
        $algaeScored = !empty($row['algae_scored']);

        $candidates[] = [
            'robot' => $robotNum,
            'matches' => $matches,
            'predicted_avg_pts_per_match' => $avgPoints, // keep old key so table still works
            'baseline_cycle' => $baselineCycle,
            'auton_score' => $autonScore,
            'most_common_action' => $favoriteAction,
            'algae_scored' => $algaeScored,
            'defensive_impact_delta' => $defImpact,
            'def_effect_cycle' => $defImpact, // placeholder local metric
            'def_effect_points' => max(0, $defImpact * 0.5) // simple local heuristic
        ];
    }

    // First pick: prioritize scoring, then auton, then faster cycle
    $firstPick = $candidates;
    usort($firstPick, function($a, $b) {
        if ($b['predicted_avg_pts_per_match'] != $a['predicted_avg_pts_per_match']) {
            return $b['predicted_avg_pts_per_match'] <=> $a['predicted_avg_pts_per_match'];
        }
        if ($b['auton_score'] != $a['auton_score']) {
            return $b['auton_score'] <=> $a['auton_score'];
        }
        return $a['baseline_cycle'] <=> $b['baseline_cycle'];
    });

    // Second pick offense: scoring + cycle speed
    $secondPickOffense = $candidates;
    usort($secondPickOffense, function($a, $b) {
        if ($b['predicted_avg_pts_per_match'] != $a['predicted_avg_pts_per_match']) {
            return $b['predicted_avg_pts_per_match'] <=> $a['predicted_avg_pts_per_match'];
        }
        return $a['baseline_cycle'] <=> $b['baseline_cycle'];
    ...
    // Defensive options: prioritize defense activity, then auton, then points
    $defensiveOptions = $candidates;
    usort($defensiveOptions, function($a, $b) {
        if ($b['defensive_impact_delta'] != $a['defensive_impact_delta']) {
            return $b['defensive_impact_delta'] <=> $a['defensive_impact_delta'];
        }
        if ($b['auton_score'] != $a['auton_score']) {
            return $b['auton_score'] <=> $a['auton_score'];
        }
    ...
    // Apply rankings
    foreach ($firstPick as $i => &$row) {
        $row['ranking'] = $i + 1;
    }
    unset($row);

    foreach ($secondPickOffense as $i => &$row) {
        $row['ranking'] = $i + 1;
    }
    unset($row);

    foreach ($defensiveOptions as $i => &$row) {
        $row['ranking'] = $i + 1;
    }
    unset($row);
    ...
    foreach ($firstPick as $i => $row) {
        $explanation = "Averages " .
            number_format($row['predicted_avg_pts_per_match'], 2) .
            " points per match, auton score " .
            number_format($row['auton_score'], 2) .
            ", estimated cycle time " .
            number_format($row['baseline_cycle'], 2) .
            " sec, favorite scoring action: " .
            $row['most_common_action'] . ".";

        if ($row['algae_scored']) {
            $explanation .= " Has scored algae.";
        }

        if ($row['defensive_impact_delta'] > 0) {
            $explanation .= " Also shows some defensive activity.";
        }
        ...
        $fullCandidateAnalysis[] = [
            ...
            'algae_scored' => $row['algae_scored']
        ];
    }

    $analysisData = [
        'first_pick_options' => array_slice($firstPick, 0, 24),
        'second_pick_options_offense' => array_slice($secondPickOffense, 0, 24),
        'defensive_options' => array_slice($defensiveOptions, 0, 24),
        'full_candidate_analysis' => $fullCandidateAnalysis
    ];
}
